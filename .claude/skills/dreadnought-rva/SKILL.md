...
`scripts/` and are run with `python`, from that directory.
...
This matters for hooking, and the `dreadnought-hooks` skill has the worked
example — the stub above swaps `rcx` and `rdx`, so hooking it with the body's
signature silently reverses your arguments.
